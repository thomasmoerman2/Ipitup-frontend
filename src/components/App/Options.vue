<template>
    <div class="flex flex-wrap justify-center w-full bg-blue-6 text-xs rounded-md p-1 gap-1.5">
        <label v-for="option in props.options" :key="option.value" :for="generateId(option.value)" class="flex flex-1 items-center gap-2 px-4 option" :class="{ 'selected': chosenOption === option.value }" @click="func_chooseOption(option.value)">
