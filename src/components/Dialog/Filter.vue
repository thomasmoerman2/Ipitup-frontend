  ...
  <div class="flex flex-col gap-6 w-full pl-3 pr-3">
    ...
    <div class="flex flex-col gap-2">
      ...
      <div class="flex flex-wrap gap-2">
        <button v-for="exercise in exerciseTypes" :key="exercise" @click="toggleExercise(exercise)" :class="['px-4 py-2 rounded-md text-sm transition-colors',
          selectedExercises.includes(exercise) ? 'bg-blue-54 text-white' : 'bg-blue-18 text-blue-84']">
          {{ exercise }}
        </button>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
      <div class="flex justify-between items-center">
        ...
        <AppSwitch v-model="bundlesOnly" id="bundles-switch" @change="handleSwitchChange" />
      </div>
      <div class="flex justify-between items-center">
        <span class="text-sm">Alleen favorieten</span>
        <AppSwitch v-model="favoritesOnly" id="favorites-switch" @change="handleSwitchChange" />
      </div>
    </div>

    <AppButton text="Toepassen" @click="applyFilters" version="1" icon="false" />
  </div>
</template>
